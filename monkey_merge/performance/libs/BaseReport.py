...
class OperateReport:
    ...
    def monitor(self, worksheet, header, data):
        ...
        worksheet.merge_range('A1:L1', 'monkey性能监控_' + header["phone_name"], define_format_H2)
        ...
        _write_center(worksheet, "H2", header["type"], self.wd)
    ...
    def plot(self,workbook, worksheet, type, lenData):
        ...
        title = ""
        if type == "cpu":
            values = '=详细信息!$A$2:$A$' + str(lenData + 1)
            row = 'I2'
            title = "cpu使用率(%)"
        elif type == "men":
            values = '=详细信息!$B$2:$B$' + str(lenData + 1)
            row = 'Q2'
            title = "内存使用(MB)"
        elif type == "fps":
            values = '=详细信息!$C$2:$C$' + str(lenData + 1)
            row = 'I30'
            title = "fps使用情况"
        elif type == "battery":
            values = '=详细信息!$D$2:$D$' + str(lenData + 1)
            row = 'I12'
            title = "电池剩余(%)"
        elif type == "flowUp":
            values = '=详细信息!$E$2:$E$' + str(lenData + 1)
            row = 'Q12'
            title = "上行流量(KB)"
        elif type == "flowDown":
            values = '=详细信息!$F$2:$F$' + str(lenData + 1)
            row = 'Q30'
            title = "下行流量(KB)"
    ...
    def crash(self, worksheet, data):
        ...
        # title = '崩溃统计日志'
        # top = workbook.add_format({'bg_color':'blue','color':'#ffffff'})
