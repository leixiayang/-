...
def stop_monkey_for_mac(device_id, device_model):
    for i in range(10):
        status, output = subprocess.getstatusoutput(adb + ' -s %s shell ps | grep monkey' % device_id)
        ...
        if output == "error: device not found":
            ...
            status, output = subprocess.getstatusoutput("%s -s %s shell kill %s" % (adb, device_id, pid))
        time.sleep(2)
